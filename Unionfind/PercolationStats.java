 ...
 *
 *  Compilation:   javac -cp .;algs4_ts.jar PercolationStats.java
 *  Execution:   java -cp .;algs4_ts.jar PercolationStats 5 30
 *
 * Summary: The PercolationStats generates a 2_D grid based on inputs.  All squares are first opened, then the program randomly generates
 * blocked and filled squares.  Filled Squares start at the top and attempts to fill each row completely before moving to the
 * next row.  Blocked squares are generated randomly, and a blocked square must opened before it can be filled.  The program 
 * takes two inputs.  NOTE: The first input is the dimensions of the 2D array.   The second input is the number of program
 ...
 *
 * createNode() creates random coordinates, so they can be flipped from (BLOCKED to OPENED), or (OPENED to FILLED).
 *
 * The nodeChangerRowTracker() method changes the squares from [blocked (0) to open (1)], with flipBlockedNodesToOpen, and from [opened (1) 
 ...
 * touching filled squares to Filled.  It also calculates statistics, which can be output to the command prompt.
 * 
...
//import edu.princeton.cs.algs4.STD.*;
//import java.awt.Font;

public class PercolationStats{
	private int dimensionHigh;
	private static final int dimensionLow=0;
	private static final int statusHigh=27;
	private static final int statusLow=25;
	private int status;
	private int[][] grid;
	private int row;
	private int col;
	...
	/***************************/
	//Static Constants have one class variable and not an instance per object
	/***************************/
	private static final int BLOCKED = 0;
	private static final int OPENED = 25;
	private static final int FILLED = 26;
	/***************************/
	...
	public PercolationStats(int dimensionHigh,int experiments)
	{
		/*Reset Values*/
		/***************************
	...
	private void resetPercolationStatsValues()
	{
		/*Reset Values*/
		/**************/
		this.fillRow=dimensionHigh-1;
	...
	{
		double PercolationStatsThreshold = (double)(this.totalFilledSquares+this.openedSquareCounter)/(double)(this.dimensionHigh*this.dimensionHigh);
		String formatedString = String.format(java.util.Locale.US,"%.2f",PercolationStatsThreshold);
		this.doubleThreshold = PercolationStatsThreshold;
		return formatedString;
	}
	public double mean(/*double[] thresholds*/)
	{
		double mean=StdStats.mean(this.doubleThresholds);
		//7/16/2017:StdOut.println("The Mean Average = "+mean);
		return mean;
	}
	private double variance(/*double[] doubleThresholds*/)
	{
		double variance = StdStats.var(this.doubleThresholds);
		//7/16/2017:StdOut.println("The Variance = "+variance);
		return variance;
	}
	public double stddev(/*double[] doubleThresholds*/)
	{
		double deviation = StdStats.stddev(this.doubleThresholds);
		//7/16/2017:StdOut.println("The Standard Deviation = "+deviation);
		return deviation;
	}
	private double runMean(/*double[] thresholds*/)
	{
		double mean=StdStats.mean(this.runTimes);
		//7/16/2017:StdOut.println("The Mean Average = "+mean);
		return mean;
	}
	private double runVariance(/*double[] doubleThresholds*/)
	{
		double runVariance = StdStats.var(this.runTimes);
		//7/16/2017:StdOut.println("The Variance = "+runVariance);
		return runVariance;
	}
	private double runStddev(/*double[] doubleThresholds*/)
	{
		double deviation = StdStats.stddev(this.runTimes);
		//7/16/2017:StdOut.println("The Standard Deviation = "+deviation);
		return deviation;
	}
	...
	private void printConfidenceInterval()
	{
		this.confidenceLo();
		this.confidenceHi();
		//7/16/2017:StdOut.println("95% confidence interval = ["+this.confidenceLow+", "+this.confidenceHigh+"]");
	}
	...
	public static void main(String[] args)
	{
		...
		double runTime;
		int dimensionArg=0;
		int experimentsArg=0;
		//while(!StdIn.isEmpty())
		//{
			dimensionArg = Integer.parseInt(args[0]);//StdIn.readInt();
			experimentsArg = Integer.parseInt(args[1]);//StdIn.readInt();
		//}
			//7/11/2017: For test Data Only: Args (dimensionHigh)
			//Args: (N, NumOfExperiments) - N,in N by N grid, Number of Experiments to run
			PercolationStats list= new PercolationStats(dimensionArg, experimentsArg);
		...
		for (int i = 0; i<list.experiments; i++)
		{
			...
			list.resetPercolationStatsValues();
		...
		//7/16/2017:StdOut.println("The number of open and filled sites are: "+list.numberOfOpenSites());
		//7/16/2017:StdOut.println("THRESHOLD STATISTICS");
		//7/16/2017:StdOut.println("********************");
		...
		//7/16/2017:StdOut.println("RUNTIME STATISTICS");
		//7/16/2017:StdOut.println("******************");
		list.runTimeMean=list.runMean(/*list.runTimes*/);
		list.runTimeVariance=list.runVariance(/*list.runTimes*/);
		list.runTimeDeviation = list.runStddev(/*list.runTimes*/);
	}//main
}//PercolationStats