...
public class Quicksort2311
{
	...
	public static Comparable[] a = new Comparable[16];
	...
	private static int partition(Comparable[] a, int lo, int hi)
	{
		...
		while(true)
		{
			...
			while(less(a[++leftScanIndex], pivot)) 
			{
				...
				if(leftScanIndex==hi) break;/*right scan*/
			}
			...
			while(less(pivot, a[--rightScanIndex])) 
			{
				...
				if(rightScanIndex == lo) break;/*left scan*/
			}
		...
		//StdOut.println("The new partition is = "+newPartition+" Left Index = "+leftScanIndex+" Lo = "+lo);
