 ...
 *  <p><b>Javadoc:</b> javadoc -author -version -private -classpath .;algs4_sts.jar; -d .\javadoc Quicksort2318_DoublingTest.java</p>
 *  <p><b>Compilation:</b>   javac -cp .;algs4_sts.jar; Quicksort2318_DoublingTest.java</p>
 *  <p><b>Execution 1:</b>     java -cp ../;.;algs4_sts.jar; sorts.Quicksort2318_DoublingTest &#60;quicksort.txt</p>
 *  <p><b>Execution 2:</b>     java -cp ../;.;algs4_sts.jar; sorts.Quicksort2318_DoublingTest &#60;qsShuffled.txt</p>
 *  <p><b>Execution 3:</b>     java -cp ../;.;algs4_sts.jar; sorts.Quicksort2318_DoublingTest &#60;distinctKeys.txt</p>
 *
 ...
 *  
 */
public class Quicksort2318_DoublingTest
{
	...
	//Note to run as a Median of 5 of 3 uncomment the one of the selections below:
	/****************************************************************************/
	...
	{
		sort(Quicksort2318_DoublingTest.a);
	}
	public static void sort(Comparable[] a)
	{
		/*StdOut.println("**Print Array before Shuffle**");
		printArray2(a);*/
		StdRandom.shuffle(a);
		/*****************************************/
		//Add sentinel value at the beginning and end of the A array
		sortFirstAndLastIndex(a);
		/*****************************************/
		//10/03/2017: 
	...
	{
		/*StdOut.println("**Print A array before partition**");
		printArray2(a);*/
		int j;
		if(hi <= lo) return;
		
		//Find a random index value of median of 3, and exchange it with the pivot position:
		/**********************************/
		int m = median(a, lo, hi, limit);
		exch(a, lo, m);
		/**********************************/
		j = partition(a,lo, hi);// sort lo, hi and partition
		
		// StdOut.println("Partition Index = "+j+" A Partition Value = "+a[j]);
		Quicksort2318_DoublingTest.sort(a, lo, j-1);//sort left side
		Quicksort2318_DoublingTest.sort(a, j+1, hi);//sort right side
		
	}
	private static int getRandomIndex(Comparable[] a, int lo, int hi, int limit)
	{		
		int medianIndex;
		int numOfElements = hi+1 - lo;
		
		//if(((hi+1)%2)==0)//even
		/*10/08: if(((numOfElements)>=limit)&&((numOfElements%2)!=0))
		{*/
			if(numOfElements >= limit)
			{
				//StdOut.println("Number of Elements = "+numOfElements);
				medianIndex = (hi+1+lo)/2;
				//10/08: int ltRtHalves = hi-medianIndex;
				int ltRtHalves = limit/2;
				//StdOut.println("**lo = "+lo+" hi = "+hi+" MedianIndex = "+medianIndex+" LtRtOffset = "+ltRtHalves);
				int leftIndex = leftEnd(ltRtHalves, medianIndex);
				int rightIndex = rightEnd(ltRtHalves, medianIndex);
				
				/*StdOut.println("**UnSorted Subrange**");
				printSubArray(Quicksort2318_DoublingTest.a, leftIndex, rightIndex);*/
				
				Insertion.sort(Quicksort2318_DoublingTest.a, leftIndex, rightIndex);
				
				/*StdOut.println("**Sorted Subrange**"+" LeftEnd = "+leftIndex+" RightEnd = "+rightIndex);
				printSubArray(Quicksort2318_DoublingTest.a, leftIndex, rightIndex);
				
				StdOut.println("**MedianIndex = "+medianIndex+" A Value = "+a[medianIndex]);*/
			}
			else
				medianIndex = lo;
	...
	public static int leftEnd(int ltRtHalves, int medianIndex)
	{
		int leftEnd = medianIndex - ltRtHalves;
		//StdOut.println("The left end = "+leftEnd);
		return leftEnd;
	}
	public static int rightEnd(int ltRtHalves, int medianIndex)
	{
		int rightEnd = medianIndex + ltRtHalves;
		//StdOut.println("The right end = "+rightEnd);
		return rightEnd;
	}
	private static void printSubArray(Comparable[] a, int leftIndex, int rightIndex)
	{
		//StdOut.println("SUBARRAY sorted:");
		//StdOut.println();
		for(int i = leftIndex; i<=rightIndex; i++)
		{
			StdOut.print(Quicksort2318_DoublingTest.a[i]+" ");
		}
		StdOut.println();
	}
	private static int median(Comparable[] a, int lo, int hi, int limit)
	{		
		//int N = a.length;
		return getRandomIndex(a, lo, hi, limit);
	}
	private static int partition(Comparable[] a, int lo, int hi)
	{
		...
		//Comparable pivot = a[medianIndex];
		while(true)
		{
			exchange = false;
			...
			while(less(a[++leftScanIndex], pivot)) 
			{
				...
				//if(leftScanIndex==hi) break;/*right scan, removing condition causes quadratic time, sentinel value is hi and sufficient*/
					//ltComparisonCtr++;
			}
			...
			//StdOut.println("leftScanIndex = "+leftScanIndex+" LeftIncrementCtr = "+leftIncrementCtr+" Exchange = "+exchange+" leftScanIncremented = "+leftScanIncremented);
					...
					{
						//10/03/2017: dualPrint(a, lo, leftScanIndex, rightScanIndex, exchange, exchange2, leftScanIncremented, rightScanDecremented, leftIncrementCtr, rightDecrementCtr, partitionCtr);
					}
					/*****************************************/
			...
			while(less(pivot, a[--rightScanIndex])) 
			{
				...
				//if(rightScanIndex == lo) break;/*left scan, removing condition causes quadratic time, sentinel value is lo and sufficient*/
					//rtComparisonCtr++;
			}
	...
	public static void sortFirstAndLastIndex(Comparable [] a)
	{
	...
	{
		/*****************************************/
		Quicksort2318_DoublingTest.printArray2(a);
		//Quicksort2318_DoublingTest.graph(a, leftScanIndex, rightScanIndex, exchange, leftScanIncremented, rightScanDecremented, leftIncrementCtr, rightDecrementCtr);
		Quicksort2318_DoublingTest.graph(a, lo, ltToRtIndexPtr, rtToLtIndexPtr, exchange, exchange2, leftScanIncremented, rightScanDecremented, leftIncrementCtr, rightDecrementCtr, partitionCtr);
		/*****************************************/
	}
	@SuppressWarnings("unchecked")
	private static void graph(Comparable[] a, int lo, int ltToRtIndexPtr, int rtToLtIndexPtr, boolean exchange, boolean exchange2, boolean leftScanIncremented, boolean rightScanDecremented, int leftIncrementCtr, int rightDecrementCtr, int partitionCtr)
	{
		...
		for (int k = 0; k< a.length; k++)
		{
			...
			{
				drawGreen(Quicksort2318_DoublingTest.tableHeight);
			...
			if(isMultLTIndexFound)
			{
				for(int i = 0; i< leftIncrementCtr; i++)
				{
					...
					drawBlack(Quicksort2318_DoublingTest.tableHeight);
					StdDraw.text(xCenter, yCenter, charConvert);
			...
			{
				drawRed(Quicksort2318_DoublingTest.tableHeight);
				StdDraw.text(xCenter, yCenter, charConvert);
			}
			if(isLTIndexExchanged)
			{
				drawBlue(Quicksort2318_DoublingTest.tableHeight);
				StdDraw.text(xCenter, yCenter, charConvert);
			}
			if(hasRTCrossedLTIndex)
			{
				drawMagenta(Quicksort2318_DoublingTest.tableHeight);
				StdDraw.text(xCenter, yCenter, charConvert);
			}
			if(hasLTCrossedRTIndex)
			{
				drawCyan(Quicksort2318_DoublingTest.tableHeight);
				StdDraw.text(xCenter, yCenter, charConvert);
			}
			if(isNoIndexFound)
			{
				drawGray(Quicksort2318_DoublingTest.tableHeight);
				StdDraw.text(xCenter, yCenter, charConvert);
			}
			...
			{
				drawOrange(Quicksort2318_DoublingTest.tableHeight);
				StdDraw.text(xCenter, yCenter, charConvert);
			}
			x1+=3.0;//X offset
		}		 
       ...
       StdDraw.pause(500);
	   Quicksort2318_DoublingTest.yOffset-=35;
	}
	...
	{
		//int Quicksort2318_DoublingTest.arrayLength = a.length;
		Quicksort2318_DoublingTest.a = new Comparable[arrayLength];
	}
	@SuppressWarnings("unchecked")
	...
	// Read strings from standard input, sort them, and print.
		...
		copyArray(Quicksort2318_DoublingTest.a, inputArray);
		/*****************************************/
		//10/03/2017: PrintArray for cmd line output and Graph for java window has been disabled for testing:
		/****************************************************************************************/
		/*Quicksort2318_DoublingTest.printArray(Quicksort2318_DoublingTest.a);
		Quicksort2318_DoublingTest.graph(a, 0, 0, 0, exchange, exchange2, false, false, 0, 0, 0);*/
		/*****************************************/
		/*StdOut.println("UNSORTED Array A values are: ");
		printArray2(a);*/
		Quicksort2318_DoublingTest.execute();
		/*StdOut.println("The left end = "+leftEnd());
		StdOut.println("The right end = "+rightEnd());
		StdOut.println("Sorted Array A values are: ");
		printArray2(Quicksort2318_DoublingTest.a);*/
	}
}//class