...
//Executable Jar: jar cvfm InsertionSort2211.jar manifest.txt InsertionSort2211.class
//compile: javac -cp .;algs4_ts.jar InsertionSort2211.java
 ...
 * @version 1.0
*/
public class InsertionSort2211
{
	...
	public static void insertionSort(Comparable[] a, int startIndex, int N)
	{
		InsertionSort2211.a = a;
		//int N = InsertionSort2211.a.length;
		int min = 0;
		//int count = 0;
		...
		{
			if(less(InsertionSort2211.a[index], InsertionSort2211.a[index-1]))
			{
				exch2(InsertionSort2211.a, index, index-1);
			//9/01/2017: StdOut.println("Max Index value = "+a[index]+" Min Index Value = "+a[index-1]+" Min Index = "+(index-1));
			}
			index--;
		}
		/************************/
		for(int rtEndMinIndex = startIndex+2; rtEndMinIndex < N; rtEndMinIndex++)
		{
			//count=0;
			exchange = false;
			//IDEA:
			...
			//j = i;
			Comparable temp = InsertionSort2211.a[rtEndMinIndex];
			int loopIndex = rtEndMinIndex;
			/************************/
			//9/01/2017: StdOut.println("************i = "+i);
			while(less(temp, InsertionSort2211.a[loopIndex-1]))
			{			
			...
			a[loopIndex]=temp;
			/************************/
			//9/6/2017: Uncomment to Graph: insertionGraph(InsertionSort2211.a, rtEndMinIndex, min, exchange);
			yOffset-=35;
		}//for
	}
	...
	private static void insertionGraph(Comparable[] a, int i, int min, boolean exchange)
	{
		...
		for (int k = 0; k< a.length; k++)
		{
			...
			if(((y2<minValue)||(iValue<y2))&&(k!=position))
			{
				...
				drawGray(InsertionSort2211.squaredDimensionHigh);
			...
			else if((y2 == iValue))
			{
				if(exchange)
				{
					drawBlack(InsertionSort2211.squaredDimensionHigh);
				}
				else if(!exchange)
				{
					drawRed(InsertionSort2211.squaredDimensionHigh);
				}
				//8/29/2017: StdDraw.filledRectangle(xCenter, yCenter, 1, (Math.abs(y2-y1))/2);
				StdDraw.text(xCenter, yCenter, charConvert);
			}
			...
			{
				drawRed(InsertionSort2211.squaredDimensionHigh);//red is the value that has been moved.
				//8/29/2017: StdDraw.filledRectangle(xCenter, yCenter, 1, (Math.abs(y2-y1))/2);
				StdDraw.text(xCenter, yCenter, charConvert);
			}
			else if(exchange&&(!((i-min)>1)&&(k==i+1)&&(less(a[i+1], a[i]))))
			{
				drawBlue(InsertionSort2211.squaredDimensionHigh);
				//8/29/2017: StdDraw.filledRectangle(xCenter, yCenter, 1, (Math.abs(y2-y1))/2);
				StdDraw.text(xCenter, yCenter, charConvert);
			}
